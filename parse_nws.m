...
% land mask created using 2022 ASIP data
landmask_ASIP = ASIPlandmask(LAT,LON);
...
for i = 1:length(yrs) % for each year
    ...
    for j = 1:length(subFolderNames)
        ...
        if dateT < datenum(2015,09,30) % last date before the subdirectory switch (this corresponds to the last loop's date)
            ...
            for k = 1:length(dys)
                ...
                icec = zeros(size(LAT,1),size(LAT,2));%nan(size(LAT,1),size(LAT,2));
                code = icec;
                rang = icec;
                areaT = icec;
                flagT = icec;
                ...
                % figure;imagescn(LON,LAT,icec); title('1')

                % now mask land
                icec(landmask_ASIP) = nan;
                rang(landmask_ASIP) = nan;
                code(landmask_ASIP) = nan;
                flagT(landmask_ASIP) = nan;
                areaT(landmask_ASIP) = nan;
        ...
        elseif dateT>=datenum(2015,09,30) && dateT<datenum(2023,01,01)  % daily subdirectories
        ...
        elseif dateT>=datenum(2023,01,01)
            ...
            for k = 1:length(dys)
                ...
                dateT = datenum(yrs(i),str2num(subFolderNames{j}),str2num(day(end-5:end-4))); % extract date information from filename
    ...
    % re-assign nans to -9999
    amt_poly(isnan(amt_poly)) = -9999;
    area(isnan(area)) = -9999;
    date(isnan(date)) = -9999;
    eggcode(isnan(eggcode)) = -9999;
    flag(isnan(flag)) = -9999;
    iceconc(isnan(iceconc)) = -9999;
    LAT(isnan(LAT)) = -9999;
    LON(isnan(LON)) = -9999;
    rangeconc(isnan(rangeconc)) = -9999;
    ...
    % save the data to a NetCDF file
        ...
        ncwrite(['nws_' num2str(yrs(i)) '.nc'],'iceconc',iceconc);
        ...
        ncwrite(['nws_' num2str(yrs(i)) '.nc'],'rangeconc',rangeconc);
        ...
        ncwrite(['nws_' num2str(yrs(i)) '.nc'],'eggcode',eggcode);
        ...
        ncwrite(['nws_' num2str(yrs(i)) '.nc'],'flag',flag);
        ...
        ncwrite(['nws_' num2str(yrs(i)) '.nc'],'area',area);
